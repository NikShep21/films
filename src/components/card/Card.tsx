...
const Card = ({ data,widthCard }: Props) => {
  ...
  if (!data) {
   ...
   >
     ...
     <div style={{aspectRatio:"487/731", width:`${widthCard}px`}} className={styles.card}>
     <div className={styles.content}>
        <div className={styles.contentContainer}>
          <Score score={data.vote_average}></Score>
          <div className={styles.genresContainer}>
            {
               data.genre_ids.slice(0,2).map((idGenre: number, id: number) => (
                  <Genre typeGenre={isMovie(data) ? 'movie' : 'tv'} idGenre={idGenre} key={id} />
                ))
            }
          </div>
